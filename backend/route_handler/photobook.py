...
class PhotobooksSettingsResponse(BaseModel):
    is_comment_enabled: bool
    is_allow_download_all_images_enabled: bool
    is_tipping_enabled: bool
...
class PhotobookAPIHandler(RouteHandler):
    def register_routes(self) -> None:
        ...
        self.route(
            "/api/photobook_settings/{photobook_id}",
    ...
    async def get_photobook_settings_by_id(
        ...
        async with self.app.new_db_session() as db_session:
            photobook_settings: DAOPhotobookSettings | None = (
                await DALPhotobookSettings.get_by_id(db_session, photobook_id)
            )
            if photobook_settings is None:
                raise HTTPException(
    ...
    async def photobook_edit_settings(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            return PhotobooksSettingsResponse(
